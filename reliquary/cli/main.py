...
def mine(
    ...
    async def _run():
        ...
        # Use 2 GPUs when available (generation on 0, GRAIL proofs on 1).
        n_gpu = torch.cuda.device_count()
        if n_gpu >= 2:
            gen_device = "cuda:0"
            proof_device = "cuda:1"
            logger.info(
                "Detected %d GPUs — dual-GPU miner: generation=%s, GRAIL=%s",
                n_gpu, gen_device, proof_device,
            )
        else:
            gen_device = proof_device = "cuda:0"
            logger.warning(
                "Only %d GPU(s) visible — generation and GRAIL will share %s",
                n_gpu, gen_device,
            )
